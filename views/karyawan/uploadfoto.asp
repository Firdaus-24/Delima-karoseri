...
uploadsDirVar = "D:\Delima\views\karyawan\Foto\"
' ****************************************************
dim ID, nip
...
function SaveFiles
    ...
    if (UBound(ks) <> -1) then
        ...
        next
		Response.Redirect("detail.asp?nip="&nip)
    else
...
<div class="container">
    <div class='row'>
        <div class='col text-center'>
            <h3>UPLOAD FOTO KARYAWAN</h3>
        </div>
    </div>
    <div class="upload">
        <form name="frmSend" method="POST" enctype="multipart/form-data" action="uploadfoto.asp?nip=<%=request.querystring("nip")%>&id=<%=id%>" onSubmit="return onSubmitForm(this);">   	<p style="margin-top: 0; margin-bottom: 0">&nbsp;</p>
        ...
        if Request.ServerVariables("REQUEST_METHOD") <> "POST" then
			...
			response.redirect "detail.asp?nip=" & request.querystring("nip")
        end if
        %>
        <u><b>Ketentuan :</b></u><ul>
        <li>Pastikan nama file Photo sudah sesuai dengan Nip karyawan bersangkutan.</li>
        <li>CONTOH : 0010821003.jpg</li>
        <li>Kami hanya menerima foto dalam bentuk format file *.jpg</li>

        <button type="button" onclick="window.location.href='detail.asp?nip=<%= nip %>'" class="btn btn-danger mt-4">Kembali</button>

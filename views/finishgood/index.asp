...
<% 
   ...
   ' ' filter agen
   ' data_cmd.commandText = "SELECT GLB_M_Agen.AgenID , GLB_M_Agen.AgenName FROM DLK_T_ProdFinishH LEFT OUTER JOIN GLB_M_Agen ON DLK_T_ProdFinishH.PFH_AgenID = GLB_M_Agen.AgenID WHERE GLB_M_Agen.AgenAktifYN = 'Y' and DLK_T_ProdFinishH.PFH_AktifYN = 'Y' GROUP BY GLB_M_Agen.AgenID, GLB_M_Agen.AgenName ORDER BY GLB_M_Agen.AgenName ASC"
   ...
   if agen <> "" then
      filterAgen = "AND DLK_T_ProdFinishH.PFH_AgenID = '"& agen &"'"
   else
      filterAgen = ""
   end if
   ...
   if tgla <> "" AND tgle <> "" then
      filtertgl = "AND dbo.DLK_T_ProdFinishH.PFH_Date BETWEEN '"& tgla &"' AND '"& tgle &"'"
   elseIf tgla <> "" AND tgle = "" then
      filtertgl = "AND dbo.DLK_T_ProdFinishH.PFH_Date = '"& tgla &"'"
   else 
      filtertgl = ""
   end if

   ' query seach 
   strquery = "SELECT dbo.DLK_T_ProdFinishH.*, dbo.GLB_M_Agen.AgenName FROM dbo.DLK_T_ProdFinishH LEFT OUTER JOIN dbo.GLB_M_Agen ON dbo.DLK_T_ProdFinishH.PFH_AgenID = dbo.GLB_M_Agen.AgenID WHERE (DLK_T_ProdFinishH.PFH_AktifYN = 'Y') OR (DLK_T_ProdFinishH.PFH_AktifYN = 'N') "& filterAgen &" "& filtertgl &""
   ' untuk data paggination
   page = Request.QueryString("page")

   orderBy = " ORDER BY PFH_Date DESC"
...
<div class="container">
   ...
   <div class="row">
      <div class="col-lg-12 mb-3">
         <a href="fin_add.asp" class="btn btn-primary ">Tambah</a>
      </div>
   </div>
   <% 'end if %>
      ...
      <div class="col-lg-12">
         <table class="table table-hover">
            ...
            <tbody>
               ...
               data_cmd.commandTExt = "SELECT PFD_ID FROM DLK_T_ProdFinishD WHERE LEFT(PFD_ID,13) = '"& rs("PFH_ID") &"'"
               set p = data_cmd.execute
               %>
                  <tr><TH><%= recordcounter %></TH>
                  <th>
                     <%= left(rs("PFH_ID"),2) %>-<%= mid(rs("PFH_ID"),3,3) %>/<%= mid(rs("PFH_ID"),6,4) %>/<%= right(rs("PFH_ID"),3)  %>
                  </th>
                  <td><%= rs("AgenNAme")%></td>
                  <td><%= Cdate(rs("PFH_Date")) %></td>
                  <td>
                     <% if session("ENG1E") = true then %>
                        <% if rs("PFH_Approve1") = "N" then %>
                           <button type="button" class="btn btn-outline-info btn-sm" data-bs-toggle="modal" data-bs-target="#modalAppProduksi" onclick="getIDProduksi('<%= rs("PFH_ID") %>', '1')">Ajukan</button>
                        <% else %>
                           Yes
                        <% end if %>
                     <% end if %>
                  ...
                  <td>
                     <% if session("ENG1E") = true then %>
                        <% if rs("PFH_Approve2") = "N" then %>
                           <button type="button" class="btn btn-outline-info btn-sm" data-bs-toggle="modal" data-bs-target="#modalAppProduksi" onclick="getIDProduksi('<%= rs("PFH_ID") %>', '2')">Ajukan</button>
                        <% else %>
                           Yes
                        <% end if %>
                     <% end if %>
                  ...
                  <td class="text-center">
                     <div class="btn-group" role="group" aria-label="Basic example">
                        <% if not p.eof then %>
                           <a href="detail.asp?id=<%= rs("PFH_ID") %>" class="btn badge text-light bg-warning">Detail</a>
                        <% end if %>
                        <% if session("ENG1B") = true then %>
                           <a href="prod_u.asp?id=<%= rs("PFH_ID") %>" class="btn badge text-bg-primary" >Update</a>
                        <% end if %>   
                        <% if session("ENG1C") = true then %>
                           <% if p.eof then %>
                              <% if rs("PFH_AktifYN") = "Y" then %>
                              <a href="aktifh.asp?id=<%= rs("PFH_ID") %>&p=N" class="btn badge text-bg-danger" onclick="deleteItem(event,'DELETE HEADER PRODUKSI')">Delete</a>
                              <% else %>
                              <a href="aktifh.asp?id=<%= rs("PFH_ID") %>&p=Y" class="btn badge text-bg-light-emphasis text-dark" onclick="deleteItem(event,'AKTIF HEADER PRODUKSI')">Aktif</a>
                              <% end if %>
                           <% end if %>
                        <% end if %>
                     </div>

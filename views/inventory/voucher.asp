...
<% 
  ...
  if agen <> "" then
    filterAgen = "AND DLK_T_VoucherH.VCH_AgenID = '"& agen &"'"
  else
    filterAgen = ""
  end if
  if dpddid <> "" then
  ...
  if tgla <> "" AND tgle <> "" then
    filtertgl = "AND dbo.DLK_T_VoucherH.VCH_Date BETWEEN '"& tgla &"' AND '"& tgle &"'"
  elseIf tgla <> "" AND tgle = "" then
    filtertgl = "AND dbo.DLK_T_VoucherH.VCH_Date = '"& tgla &"'"
  else 
    filtertgl = ""
  end if
    ...
    <div class="row">
      ...
      <div class="col-lg-4 mb-3">
        ...
        <select class="form-select" aria-label="Default select example" name="dpddid" id="dpddid">
          ...
          <option value="<%= pddid("VCH_PDDID") %>"><%= left(pddid("VCH_PDDID"),2) %>-<%= mid(pddid("VCH_PDDID"),3,3) %>/<%= mid(pddid("VCH_PDDID"),6,4) %>/<%= mid(pddid("VCH_PDDID"),10,4) %>/<%= right(pddid("VCH_PDDID"),3) %></option>
          <% 
          Response.flush
          pddid.movenext
          loop

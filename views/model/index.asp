...
<% 
    ...
    data_cmd.CommandText = "SELECT AgenID, AgenName FROM DLK_M_Barang LEFT OUTER JOIN GLB_M_Agen ON LEFT(DLK_M_BArang.Brg_ID,3) = GLB_M_Agen.AgenID WHERE BRg_AktifYN = 'Y' AND Brg_Type = 'T01' OR Brg_Type = 'T02' GROUP BY AgenID, AgenName ORDER BY AgenName ASC"
    ...
    ' real query
    strquery = "SELECT DLK_M_Barang.*, DLK_M_TypeBarang.T_Nama, DLK_M_kategori.KategoriNama, DLK_M_JenisBarang.jenisNama FROM DLK_M_Barang LEFT OUTER JOIN DLK_M_TypeBarang ON DLK_M_Barang.Brg_Type = DLK_M_TypeBarang.T_ID LEFT OUTER JOIN DLK_M_KAtegori ON DLK_M_Barang.kategoriID = DLK_M_Kategori.kategoriID LEFT OUTER JOIN DLK_M_JenisBarang ON DLK_M_Barang.jenisID = DLK_M_JenisBarang.JenisID WHERE Brg_AktifYN = 'Y' "& filterNama &""& filterKat &""& filterJen &" "& filterAgen &" AND (Brg_type = 'T01' OR Brg_type = 'T02')"
    ' untuk data paggination
    page = Request.QueryString("page")


    orderBy = " order by Brg_id, Brg_Nama ASC"
...
<div class="container">
    <div class="row mt-3">
        <div class="col-lg-12 text-center">
            <h3>MASTER MODEL & SUBPART</h3>
        </div>
    </div>
    <div class="d-flex mt-3 mb-3">
      <% if session("MDL1A") = true then%>
      <div class="me-auto p-2">
        <a href="m_add.asp" class="btn btn-primary">
          Tambah
        </a>
      </div>
      <% end if %>
      <div class="p-2">
        ...
        <% if session("M3") = true then%>
          <a href="../jenis/" class="btn btn-danger">
            Jenis
          </a>
        <% end if %>
      </div>
    ...
    <div class="row">
        ...
        <form action="index.asp" method="post">
            <input type="text" class="form-control" name="nama" id="nama" autocomplete="off" placeholder="cari nama model">
        </div>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <div class="btn-group" role="group" aria-label="Basic example">
                                <% if session("MDL1B") = true then%>
                                    <a href="m_u.asp?id=<%= rs("brg_id") %>" class="btn badge text-bg-primary">update</a> 
                                <% end if %>
                                <% if session("MDL1B") = true then%>
                                    <a href="aktif.asp?id=<%= rs("brg_id") %>" class="btn badge text-bg-danger" onclick="deleteItem(event, 'HAPUS MASTER MODEL')">delete</a>
                                <% end if %>
                            </div>
                        </td>
                    </tr>
        ...
        <div class="col-sm-12">
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    ...
                    %>
                        <a class="page-link prev" href="./?offset=<%= requestrecords - recordsonpage%>&page=<%=npage%>&nama=<%= nama %>&ket=<%= ket %>&jen=<%= jen %>&pagen=<%= pagen %>">&#x25C4; Prev </a>
                    <% else %>
                    ...
                    <li class="page-item d-flex" style="overflow-y:auto;height: max-content;">	
                        ...
                        %>
                            <a class="page-link hal bg-primary text-light" href="./?offset=<% = pagelist %>&page=<%=pagelistcounter%>&nama=<%= nama %>&ket=<%= ket %>&jen=<%= jen %>&pagen=<%= pagen %>"><%= pagelistcounter %></a> 
                        <%else%>
                            <a class="page-link hal" href="./?offset=<% = pagelist %>&page=<%=pagelistcounter%>&nama=<%= nama %>&ket=<%= ket %>&jen=<%= jen %>&pagen=<%= pagen %>"><%= pagelistcounter %></a> 
                        <%
                        end if
                    ...
                    <li class="page-item">
                        ...
                        <% if(recordcounter > 1) and (lastrecord <> 1) then %>
                            <a class="page-link next" href="./?offset=<%= requestrecords + recordsonpage %>&page=<%=page%>&nama=<%= nama %>&ket=<%= ket %>&jen=<%= jen %>&pagen=<%= pagen %>">Next &#x25BA;</a>
                        <% else %>

...
<div class="container">
    ...
    <form action="bank_u.asp?id=<%= id %>" method="post" onsubmit="validasiForm (this,event,'Master Bank','info')">
        ...
        <div class="row  mb-3 mt-3 d-flex justify-content-center">
            ...
            <div class="col-sm-5">
               <input type="text" class="form-control" name="keterangan" id="keterangan" autocomplete="off" maxlength="50" value="<%= data("Bank_Keterangan") %>">
            </div>
        </div>
...
if Request.ServerVariables("REQUEST_METHOD") = "POST" then 
   ...
   data_cmd.CommandText = "SELECT * FROM GL_M_Bank WHERE UPPER(Bank_Name) = '"& ucase(nama) &"' AND Bank_aktifYN = 'Y'"

   set ddata = data_cmd.execute

   if ddata.eof then
      call query("UPDATE GL_M_Bank SET Bank_name = '"& nama &"', Bank_keterangan = '"& keterangan &"', Bank_UpdateID  = '"& session("userid") &"', Bank_UpdateTime = '"& now &"' WHERE Bank_ID = '"& trim(Request.Form("id")) &"'")   

      call alert("MASTER BANK", "berhasil update", "success","index.asp") 
   else

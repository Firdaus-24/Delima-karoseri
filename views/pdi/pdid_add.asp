...
<div class="container">
  ...
  <form action="pdid_add.asp?id=<%= id %>" method="get" style="position:relative">
          ...
          <tr>
            ...
            <td class="text-center">
              <div class="btn-group" role="group" aria-label="Basic example">
                <a href="aktifd.asp?id=<%= rs("PDI_ID") %>&init=<%= rs("PDI_Description") %>&p=pdid_add" class="btn badge text-bg-danger" onclick="deleteItem(event,'hapus detail PDI')">Delete</a>
              </div>
            </td>
          </tr>
          <tr>
    ...
    <div class="col-sm-12">
      ...
      <nav aria-label="Page navigation example">
        <ul class="pagination">
          ...
          %>
            <a class="page-link prev" href="pdid_add.asp?offset=<%= requestrecords - recordsonpage%>&page=<%=npage%>&id=<%=id%>&ckey2=<%=ckey2%>&condition=<%=condition%>">&#x25C4; Prev </a>
          <% else %>
          ...
          %>
            <a class="page-link hal bg-primary text-light" href="pdid_add.asp?offset=<% = pagelist %>&page=<%=pagelistcounter%>&id=<%=id%>&ckey2=<%=ckey2%>&condition=<%=condition%>"><%= pagelistcounter %></a> 
          <%else%>
            <a class="page-link hal" href="pdid_add.asp?offset=<% = pagelist %>&page=<%=pagelistcounter%>&id=<%=id%>&ckey2=<%=ckey2%>&condition=<%=condition%>"><%= pagelistcounter %></a> 
          <%
          end if
          ...
          <li class="page-item">
            ...
            <% if(recordcounter > 1) and (lastrecord <> 1) then %>
              <a class="page-link next" href="pdid_add.asp?offset=<%= requestrecords + recordsonpage %>&page=<%=page%>&id=<%=id%>&ckey=<%=ckey2%>&condition=<%=condition%>">Next &#x25BA;</a>
            <% else %>
...
<div class="modal fade" id="modalDetailPdi" tabindex="-1" aria-labelledby="modalDetailPdiLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <form action="pdid_add.asp?id=<%= data("PDI_ID") %>" method="post">

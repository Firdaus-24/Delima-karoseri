          ...
          <tr>
            ...
            <td class="text-center">
              <div class="btn-group" role="group" aria-label="Basic example">
                <a href="aktifd.asp?id=<%= rs("PDI_ID") %>&init=<%= rs("PDI_Initial") %>&p=pdid_add" class="btn badge text-bg-danger" onclick="deleteItem(event,'hapus detail PDI')">Delete</a>
              </div>
            </td>
          </tr>
          <tr>
    ...
    <div class="col-sm-12">
      ...
      <nav aria-label="Page navigation example">
        <ul class="pagination">
          ...
          %>
            <a class="page-link prev" href="pdid_add.asp?offset=<%= requestrecords - recordsonpage%>&page=<%=npage%>&id=<%=id%>">&#x25C4; Prev </a>
          <% else %>
          ...
          %>
            <a class="page-link hal bg-primary text-light" href="pdid_add.asp?offset=<% = pagelist %>&page=<%=pagelistcounter%>&id=<%=id%>"><%= pagelistcounter %></a> 
          <%else%>
            <a class="page-link hal" href="pdid_add.asp?offset=<% = pagelist %>&page=<%=pagelistcounter%>&id=<%=id%>"><%= pagelistcounter %></a> 
          <%
          end if
          ...
          <li class="page-item">
            ...
            <% if(recordcounter > 1) and (lastrecord <> 1) then %>
              <a class="page-link next" href="pdid_add.asp?offset=<%= requestrecords + recordsonpage %>&page=<%=page%>&id=<%=id%>">Next &#x25BA;</a>
            <% else %>

...
<% 
  ...
  if left(ojhid,2) = "SO" then
    data_cmd.commandText = "SELECT CustNama, custID FROM MKT_T_OrjulH LEFT OUTER JOIN DLK_M_Customer ON MKT_T_OrjulH.OJH_Custid = DLK_M_Customer.custID WHERE OJH_ID = '"& ojhid &"' AND OJH_AktifYN = 'Y' ORDER BY OJH_ID ASC"
  else

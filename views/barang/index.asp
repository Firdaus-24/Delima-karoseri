...
<% 
    ...
    ' query seach 
    if nama <> "" then
        filterNama = " AND Ucase(Brg_Nama) LIKE '%"& nama &"%' "
    end if
    if ven <> "" then
        filterVen = " AND Brg_VendorID = '"& ven &"'"
    end if
    if kat <> "" then
    ...
    ' real query
    strquery = "SELECT * FROM DLK_M_Barang WHERE Brg_AktifYN = 'Y' "& filterNama &""& filterVen &""& filterKat &""& filterJen &""

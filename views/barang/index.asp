...
<% 
    ...
    ' real query
    strquery = "SELECT DLK_M_Barang.*, DLK_M_TypeBarang.T_Nama, DLK_M_kategori.KategoriNama, DLK_M_JenisBarang.jenisNama FROM DLK_M_Barang LEFT OUTER JOIN DLK_M_TypeBarang ON DLK_M_Barang.Brg_Type = DLK_M_TypeBarang.T_ID LEFT OUTER JOIN DLK_M_KAtegori ON DLK_M_Barang.kategoriID = DLK_M_Kategori.kategoriID LEFT OUTER JOIN DLK_M_JenisBarang ON DLK_M_Barang.jenisID = DLK_M_JenisBarang.JenisID WHERE Brg_AktifYN = 'Y' AND Brg_type <> 'T01' AND Brg_type <> 'T02' AND Brg_type <> 'T05' AND Brg_type <> 'T06' "& filterNama &""& filterKat &""& filterJen &" "& filterAgen &""

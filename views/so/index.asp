         ...
         <table class="table table-hover">
            ...
            <tbody>
               ...
               %>
                  ...
                  <th>
                     <%= left(rs("OJH_ID"),2) %>-<%= mid(rs("OJH_ID"),3,3) %>/<%= mid(rs("OJH_ID"),6,4) %>/<%= right(rs("OJH_ID"),4)  %>
                  </th>
                  <td><%= rs("AgenNAme")%></td>
                  <td><%= Cdate(rs("OJH_Date")) %></td>
                  <td><%= rs("custNama")%></td>
                  <td>
                     <% if Cdate(rs("OJH_JTDate")) <> Cdate("1/1/1900") then%> 
                        <%= cdate(rs("OJH_JTDate")) %>
                     <% end if %>
                  </td>
                  <td><%= rs("OJH_DiskonALL")%></td>
                  <td><%= rs("OJH_PPN")%></td>
                  <td><%= rs("OJH_TimeWork")%></td>
                  <td class="text-center">
                     <div class="btn-group" role="group" aria-label="Basic example">
                        <% if not p.eof then %>
                           <a href="detail.asp?id=<%= rs("OJH_ID") %>" class="btn badge text-light bg-warning">Detail</a>
                        <% end if %>
                        <% if session("MK1B") = true then %>
                           <a href="so_u.asp?id=<%= rs("OJH_ID") %>" class="btn badge text-bg-primary" >Update</a>
                        <% end if %>   
                        <% if session("MK1C") = true then %>
                           <% if p.eof then %>
                              <a href="aktifh.asp?id=<%= rs("OJH_ID") %>" class="btn badge text-bg-danger" onclick="deleteItem(event,'DELETE SALES ORDER')">Delete</a>
                           <% end if %>
                        <%end if %>
                     </div>
                  </td>

...
sub tambahDelbarang()
    ...
    data_cmd.CommandText = "SELECT * FROM DLK_T_DelBarang WHERE DB_AgenID = '"& delcabang &"' AND DB_Date = '"& tgl &"' AND DB_Item = '"& brg &"' AND DB_Acc1 = '"& acc1 &"' AND DB_Acc2 = '"& acc2 &"'"
...
sub updateDelbarang()
    ...
    if not ddata.eof then
        call query("UPDATE DLK_T_DelBarang SET DB_AgenID = '', DB_Date = '', DB_Item = '',DB_IPDIPHID = '', DB_QtySatuan = 0, DB_Harga = '', DB_JenisSat = '', DB_Keterangan = '',DB_Acc1 = '', DB_Acc2 = '' WHERE DB_Id = '"& id &"'")
        ...
        if angka > 0 then
            ...
            if angka > Cint(qty) then
                ...
                nilai = false 'flage untuk cek quantyty
                else
                call query ("sp_AddDLK_T_DelBarang '"& delcabang &"', '"& tgl &"', '"& brg &"', '"& pembelian("IPD_IPHID") &"','"& angka &"', '"& tharga &"', '"& satuan &"', '"& ket &"', '','','','"& acc1 &"','"& acc2 &"'")
                end if
                qty = Cint(qty) - angka

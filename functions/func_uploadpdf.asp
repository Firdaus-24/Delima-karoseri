...
Dim uploadsDirVar, pathidh
pathidh = trim(Request.QueryString("pathidh"))
  response.AddHeader "REFRESH","0:URL=index.asp?id=000000000"
uploadsDirVar = pathDoc & pathidh & "\"
' ****************************************************
function successUpload(description, strcoment, icon)
  call header("UPLOAD DOCUMENT")
    call alert("UPLOADING PDF", description & strcoment, icon, responback )
  call footer()
end function
...
function SaveFiles
  ...
  if (UBound(ks) <> -1) then
    for each fileKey in Upload.UploadedFiles.keys
      SaveFiles = successUpload("Berhasil diupload ", SaveFiles & Upload.UploadedFiles(fileKey).FileName & " (" & Upload.UploadedFiles(fileKey).Length & "B) ", "success")
    next
  else
    SaveFiles = successUpload("tidak bisa upload ", "", "error")
  end if
...
Private nameOfFile
  ...
  Public Property Get FileName()
      FileName = request.querystring("id") & ".pdf"
  End Property
